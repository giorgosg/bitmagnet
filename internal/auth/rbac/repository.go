...
func (r *repository) PutRole(ctx context.Context, role Role, objectActions []ObjectAction) (RoleInfo, error) {
	...
	err := r.dao.DaoTransaction(func(tx *dao.Query) error {
		...
		// PutRole replaces the role's permission set. An empty set is a revocation
		// of everything, so the delete runs unconditionally — returning early here
		// would silently leave the old permissions in place.
